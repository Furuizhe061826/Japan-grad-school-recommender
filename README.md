...
哲思学院旗下的作品集型 Next.js MVP。用户填写本科院校背景、研究方向、GPA、语言成绩和申请偏好后，系统会基于本地研究科/方向 JSON 数据与规则算法，生成「冲刺 / 匹配 / 相对稳妥」三档日本大学院推荐结果。用户也可以填写“意向院校”，系统会单独判断该目标学校的申请可行性，并给出校内更匹配的项目方向、潜在教授/研究室、风险点和准备建议。

首页已品牌化为「哲思学院 Zhesi Academy」，同时保留 Portfolio MVP 标识，便于放入简历和作品集链接中展示。
...
9. 如果用户填写意向院校，系统会先识别中英文校名和常见简称，再基于该校已收录项目计算申请可行性，输出更匹配的研究科/项目、教授/研究室、优势、风险和准备建议。
10. 为每条推荐生成评分解释、数据可信度、募集要项核验状态和教授库覆盖提示。
11. 如果没有强命中教授，系统会解释可能原因，而不是直接空白展示。
12. 输出冲刺、匹配、相对稳妥三档推荐，并生成自然语言理由和提升建议。
